...
- ⚠️ **활성화엔 설정 필요:** 카카오 개발자 앱 등록 + Supabase Kakao provider + Redirect URLs 허용목록. 가이드 참고.
...
### Fixed (Obsidian 이스케이프 — 2026-06-01)
- **Obsidian `==` 이스케이프 비-lossy화.** 본문에 `==`가 있을 때 보이는 공백을 넣어(`= =`) 텍스트를 손상시키던 걸 → 연속된 `=` 사이에 제로폭 공백(U+200B)을 끼워 화면엔 그대로 보이면서 하이라이트가 깨지지 않게. 회귀 테스트 추가.

### Security (Worker 하드닝 — 2026-06-01, 미배포)
- **전역 한도 DoS 차단.** 전역 일일 카운터를 인증된 모든 요청마다 올리던 걸 → **실제 크레딧 차감 성공 시에만** 카운트. 악의적 로그인 사용자가 빈 요청을 반복해 전체 서비스 한도를 소진(타 사용자 429)시키지 못하게. (`checkGlobalCap` → `globalCapExceeded` 읽기 체크 + `bumpGlobal`)
- **내부 에러 메시지 노출 제거.** 크레딧 RPC 실패 시 502 응답에 Supabase 원문 에러를 echo하던 걸 일반 안내로 교체, 상세는 서버 로그(`console.log`)로만.
...
- **처리 화면 완료/취소 경합 가드.** ProcessingView가 `handled` 플래그로 `onFinish`·`onCancel`을 정확히 한 번만 발화 — 완료 순간 취소를 탭하면 둘 다 불려 네비게이션이 꼬이던 좁은 경합 제거.
...
- **크레딧 UI 노출.** 홈 헤더에 잔액 펠(Lumark Cloud+로그인 시, 탭→설정), 결과 화면 "퀴즈 만들기" 버튼에 비용 배지(2 크레딧). 본인 키 엔진이면 비용 표시 숨김(무제한).
- **크레딧 부족 시 "내 Gemini 키로 전환" 버튼.** 크레딧 부족 알림(코드 "CREDITS")에 액션 추가 — 누르면 OCR 엔진을 즉시 본인 키로 바꿔 무제한 사용. `errorAlert` 모디파이어에서 공통 처리(OCR 사전 게이트·OCR 402·퀴즈 402 모든 경로 적용).
...
- **"내 퀴즈" 행 다듬기.** 날짜 한국어 표기(M월 d일, RecentNoteRow와 통일) + 첫 질문 미리보기 한 줄.
