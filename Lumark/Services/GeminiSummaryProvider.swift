...
//  Lumark
//
//  본인 Gemini 키로 직접 텍스트 → AI 정리본 생성. (Lumark 크레딧 미사용)
//  GeminiQuizProvider와 동일 구조, SummaryPrompt 사용.
//
...
struct GeminiSummaryProvider: SummaryProvider {
    ...
    func generate(from text: String) async throws -> String {
        ...
        let body: [String: Any] = [
            ...
            "generationConfig": [
                ...
                "responseSchema": SummaryPrompt.schema(),
                "maxOutputTokens": 8192,   // 정리본은 출력이 길다.
                "temperature": 0.2,
            ],
        ]
    ...
    /// candidates[0].content.parts[0].text(JSON 문자열) → 마크다운.
    nonisolated static func parseGeminiResponse(_ data: Data) throws -> String {
        ...
        return try SummaryPrompt.parse(innerData)
    }
}
